...
/**
 * Check that lb Load Balancers do not allow unencrypted (HTTP) traffic.
 *
 * @severity critical
...
export const disallowUnencryptedTraffic: ResourceValidationPolicy = policyManager.registerPolicy({
    resourceValidationPolicy: {
        ...
        description: "Check that LB Load Balancers do not allow unencrypted (HTTP) traffic.",
        ...
        validateResource: validateResourceOfType(Listener, (listener, args, reportViolation) => {
            if ((listener.port && listener.port === 80) || (listener.protocol && listener.protocol === "HTTP")) {
                reportViolation("LB Load Balancers should now allow unencrypted (HTTP) traffic.");
            }
        }),
    },
    vendors: ["aws"],
